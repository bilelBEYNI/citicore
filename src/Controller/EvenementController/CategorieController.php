...
class CategorieController extends AbstractController
{
    ...
    #[Route('/dashboard/categorie/delete/{id}', name: 'app_categorie_delete', methods: ['POST'])]
    public function delete(int $id, CategorieRepository $categorieRepository, EntityManagerInterface $em, Request $request): Response
    {
        $categorie = $categorieRepository->find($id);
        ...
        if (!$categorie->getEvenements()->isEmpty()) {
            $this->addFlash('error', 'Impossible de supprimer la catégorie car elle contient des événements.');
            return $this->redirectToRoute('app_categorie_show', ['id' => $id]);
        }

        // Delete the image file if it exists
        if ($categorie->getImageUrl()) {
            $imagePath = $this->getParameter('uploads_directory').'/'.$categorie->getImageUrl();
    ...
    private function handleImageUpload($form, Categorie $categorie, SluggerInterface $slugger): void
    {
        ...
        if ($imageFile) {
            ...
            try {
                $imageFile->move(
                    $this->getParameter('uploads_directory'), // Changed to match the add method
                    $newFilename
                );
                
                // Delete old image if it exists
                if ($categorie->getImageUrl()) {
                    $oldImagePath = $this->getParameter('uploads_directory').'/'.$categorie->getImageUrl();
